    ...
    */ 
    ...
    pose checkWhichNeighbour(const Point & parent){
        for(int i =0; i < NUMBER_OF_POSE; i++){
            if(neighbourList[i].x == parent.x && neighbourList[i].y == parent.y)
                return pose(i);
            else
                return pose(0);
        }
    ...
    int checkBlockedCorridor(const Point& _p,const pose& _pos){
        ...
        if(_pos == right || _pos == left) return maze.isFree(_p.x-1,_p.y) + maze.isFree(_p.x+1,_p.y);
        else return maze.isFree(_p.x,_p.y-1) + maze.isFree(_p.x,_p.y+1);
    }
    ...
    pose whichCorrPosNotBlocked(const Point& _p,const pose& _pos){
        ...
        if(_pos == right || _pos == left){
            if(maze.isFree(_p.x-1,_p.y) == 1) return pose(up);
            if(maze.isFree(_p.x+1,_p.y) == 1) return pose(down);
        }
        else{
            if(maze.isFree(_p.x,_p.y-1) == 1) return pose(left);
            if(maze.isFree(_p.x,_p.y+1) == 1) return pose(right);
        }
    }
    ...
    bool isCorridorOrCorner(const Point& _p, pose& _pos){
        ...
        if(maze.isFree(frontNeighbour.x,frontNeighbour.y) == 0){
    ...
    bool getTrueNeighbour(Point& _trueNeighibour, pose _pos, int& _move_distance){
        ...
        if(maze.isFree(immediateParentNeighbour.x,immediateParentNeighbour.y) == 0)return false;
        else{
